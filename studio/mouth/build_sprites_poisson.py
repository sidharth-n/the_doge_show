...
def mouth_mask(img):
    ...
    n,lab,stats,cent=cv2.connectedComponentsWithStats(m); cx,cy=(BOX[0]+BOX[2])//2,(BOX[1]+BOX[3])//2-20
    best=None
    for i in range(1,n):
...
for i,n in enumerate(ORDER):
    ...
    blended=cv2.seamlessClone(im,base,m,center,cv2.NORMAL_CLONE)
    ...
    rgba=cv2.resize(rgba,(round(bw*SC),round(bh*SC)),interpolation=cv2.INTER_AREA); cv2.imwrite(f'{OUT}/v{i}.png',rgba)
